...
then
    if ! [ -e "mspbwt/tests/testthat/test-${what_to_test}.R" ] && ! [ -e "mspbwt/tests/testthat/test-${what_to_test}.R" ]
    then
...
R -e 'devtools::document("mspbwt"); devtools::load_all("mspbwt", quiet = FALSE); devtools::test("mspbwt", filter = "'${what_to_test}'", reporter = "summary")' 2>&1 | tee ${logfile}

# print(getwd()); library(devtools); load_all("mspbwt", quiet = FALSE);

# somehow this gives 0 exit code on parse failure
started_if_1=`cat ${logfile} | grep 'Testing mspbwt' | wc -l`
